...
echo -e "${CYAN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║     Gallery Curator — Orchid Demo           ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
case "$mode" in
  ...
  2)
    ...
    echo ""
    echo -e "${YELLOW}Make sure Qdrant is running if using rag_augmented memory.${NC}"
    echo ""
    ORCHID_CONFIG=examples/gallery_curator/orchid.yml uvicorn orchid_api.main:app --host 0.0.0.0 --port 8000
    ;;
  3)
    echo ""
